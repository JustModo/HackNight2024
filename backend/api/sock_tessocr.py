...
def detect_character():
    global detected_text, last_ocr_time, prev_cx, prev_cy
            ...
            if landmarks[finger_tips[1]][2] < landmarks[finger_tips[1] - 2][2]:  # Index finger is up
                ...
                if current_time - last_ocr_time > ocr_interval and (cx != prev_cx or cy != prev_cy):
                    ...
                    if len(text.strip()) == 1:
                        ...
                        # Emit detected character via SocketIO
                        socketio.emit('detected_character', {
                                      'character': text.strip(), 'text': detected_text})
...
def handle_start_ocr():
    socketio.start_background_task(target=detect_character)


# Flask-SocketIO main loop
